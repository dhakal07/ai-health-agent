...
export const API_BASE = envBase && envBase.trim() ? envBase.trim() : "http://localhost:8000";  // ← localhost, not 127.0.0.1
...
/**
 * Chat with backend + SEND HISTORY
 */
export async function chat(input, history = []) {
  let location = null;
  try {
  ...
  return await fetchJSON(`${API_BASE}/chat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message: input, location, history }),
  });
}