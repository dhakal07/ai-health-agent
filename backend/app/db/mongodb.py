...
DB_MODE = "memory"  # will flip to "mongo" after successful ping
...
# ---- try real Mongo first; fall back to memory quickly ----
try:
    _client = MongoClient(
        settings.MONGO_URI,
        serverSelectionTimeoutMS=2500,  # short, so UI never hangs
        connectTimeoutMS=2500,
        tlsCAFile=certifi.where(),      # <-- IMPORTANT on macOS for Atlas TLS
    )
    _client.admin.command("ping")      # raises if cannot connect
